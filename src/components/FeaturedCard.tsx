...
export default function FeaturedCard({ project, num }: { project: Project; num: string }) {
  return (
    <a
      ...
      className="panel disc-hover group block overflow-hidden hover:border-green"
    >
      <div className="relative aspect-[16/9] overflow-hidden border-b border-ink-600">
        {project.cover ? (
          ...
          <div className="node-grid h-full" />
        )}
        <div className="absolute inset-x-0 top-0 flex items-start justify-between p-3">
          <span className="bg-ink-950/90 px-2 py-1 font-mono text-[10px] uppercase tracking-wideish text-greenBright">
            case study {num}
          </span>
          {project.status && (
            <span className="bg-ink-950/90 px-2 py-1 font-mono text-[10px] uppercase tracking-wideish text-muted">
              {project.status}
            </span>
          )}
        </div>
      ...
      <div className="p-4 sm:p-5">
        ...
        <h3 className="mt-1.5 font-display text-xl font-black uppercase leading-tight tracking-tight text-paper transition-colors group-hover:text-green sm:text-2xl">
          {project.title}
        </h3>
        {project.subtitle && (
          <p className="mt-1 font-mono text-[11px] uppercase tracking-wideish text-green">
            {project.subtitle}
          </p>
        )}
        ...
        {project.chain && (
          <p className="mt-3 border-t border-ink-600 pt-3 font-mono text-[10px] uppercase tracking-[0.12em] text-paper/70">
            {project.chain}
          </p>
        )}
        <div className="mt-3">
